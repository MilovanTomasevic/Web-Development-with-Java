...
public class NovaKnjigaServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        String brSlobodnih = request.getParameter("brojprimeraka");

        if (isbn.trim().equals("") || naslov.trim().equals("") || autor.trim().equals("") || brSlobodnih.trim().equals("")) {
            Poruka poruka = new Poruka("!unetapolja");
            session.setAttribute("poruka", poruka);
            RequestDispatcher disp = request.getRequestDispatcher(
                    "view/NovaKnjigaResult.jsp");
            disp.forward(request, response);
            return;
        }

        out.println("unosKnjige÷" + isbn + "÷" + naslov + "÷" + autor + "÷" + brSlobodnih);
        Poruka poruka = new Poruka(in.readLine());
        Knjiga knjiga = new Knjiga(isbn, naslov, autor, 0);
